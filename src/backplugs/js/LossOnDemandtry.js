            ...
            if (data != "") {
                ...
                if (response.length > 0) {
                    ...
                    for (var e = 0; e < tDate.length; e++) {
                        ...
                        var Shift = [...new Set(result.map(x => x.sShift))];
                        for (var d = 0; d < Shift.length; d++) {
                            var shiftres = result.filter(ro => String(ro.sShift).includes(Shift[d]));
                            if (shiftres.length > 1) {
                                var row = '<tr style="cursor:pointer;" class="xyzshift">';
                                row += '<td>' + '' + '</td>';
                                var st = result[0]["tDate"];
                                var st1 = st.split("T");
                                var dt = st1[0];
                                ...
                                row += '<td>' + dt + '</td>';
                                row += '<td>' + shiftres[0]["sShift"] + '</td>';
                                row += '<td>' + shiftres[0]["WorkcellDesc"] + '</td>';
                                row += '<td>' + shiftres[0]["MMPCodeDesc"] + '</td>';
                                row += '<td>' + shiftres[0]["MMPLossCodeDesc"] + '</td>';
                                row += '<td>' + shiftres[0]["LossDesc"] + '</td>';
                                row += '<td>' + Math.round(shiftres[0]["TotalSec"] / 60) + '</td>';
                                row += '<td>' + Math.round(shiftres[0]["AverageSec"] / 60) + '</td>';
                                row += '<td>' + Math.round(shiftres[0]["MinimumSec"] / 60) + '</td>';
                                row += '<td>' + Math.round(shiftres[0]["MaximumSec"] / 60) + '</td>';
                                row += '<td>' + Math.round(shiftres[0]["TotalCount"]) + '</td>';
                                row += '</tr>';
                            }
                            $('#EventDetailBody').append(row);

                            for (var i = 0; i < shiftres.length; i++) {
                                var row1 = '<tr class="abc" style="display:none">';
                                var st = shiftres[i]["tDate"];
                                var st1 = st.split("T");
                                var dt = st1[0];
                                var sr = i + 1
                                row1 += '<td>' + sr + '</td>';
                                row1 += '<td>' + dt + '</td>';
                                row1 += '<td>' + shiftres[i]["sShift"] + '</td>';
                                row1 += '<td>' + shiftres[i]["WorkcellDesc"] + '</td>';
                                row1 += '<td>' + shiftres[i]["MMPCodeDesc"] + '</td>';
                                row1 += '<td>' + shiftres[i]["MMPLossCodeDesc"] + '</td>';
                                row1 += '<td>' + shiftres[i]["LossDesc"] + '</td>';
                                row1 += '<td>' + Math.round(shiftres[i]["TotalSec"] / 60) + '</td>';
                                row1 += '<td>' + Math.round(shiftres[i]["AverageSec"] / 60) + '</td>';
                                row1 += '<td>' + Math.round(shiftres[i]["MinimumSec"] / 60) + '</td>';
                                row1 += '<td>' + Math.round(shiftres[i]["MaximumSec"] / 60) + '</td>';
                                row1 += '<td>' + Math.round(shiftres[i]["TotalCount"]) + '</td>';
                                row1 += '</tr>';
                    ...
                    $(function () {
                        ...
                        $("#tblEventDetail").click(function (event) {
                            ...
                            } else if ($target.closest("tr").attr("class") == "xyzshift") {
                                var date = $target.closest("tr").next()[0].childNodes[1]["textContent"]
                                var result = response.filter(ro => String(ro.tDate).includes(date));

                                if (result.length > 1) {
                                    var shift = $target.closest("tr").next()[1].childNodes[1]["textContent"]
                                    var shiftresult = result.filter(ro => String(ro.sShift).includes(shift));
                                    if (shiftresult.length > 1) {
                                        $target.closest("tr").nextUntil(" tr.xyzshift").slideToggle()
